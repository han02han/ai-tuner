...
旧版（纯 Mel 频谱 + pyrubberband 生成数据）的训练出现过一个典型失败模式：除了第一个 epoch，之后的 epoch loss 几乎不再变化，模型学了恒等映射——"什么都不做，把输入 Mel 原样输出"。原因是 Mel 频谱既是输入也是输出的表示空间，加上 pyrubberband 频域拉伸生成的数据本身就是可逆操作，模型很快找到了 trivial solution，梯度信号消失。

新版用 **HuBERT 把输入空间彻底变了**——模型接收的是 768 维语义特征（"在唱什么"），不是频谱。它没法偷懒做恒等映射，因为 HuBERT 特征和波形之间差了一个完整的生成过程。模型必须学会：

1. 从 HuBERT 语义特征理解内容
2. 从 pitch 条件理解目标音高
3. 通过 HiFi-GAN 上采样重建波形

这是真正的生成任务，不是"撤销 DSP"，梯度信号会持续有意义。训练时可以观察 `loss_mel` 是否稳步下降（>0.5）来验证这一点——如果第一个 epoch `loss_mel` 就 <0.3，说明数据太简单或模型又在偷懒。
...
`TrainConfig.segment_ms` 控制训练时从数据片段中随机切出的长度，默认 800ms（适配 12GB 最低配）。但推理时面对的是完整歌曲（3-5 分钟），存在训练-推理分布不匹配——模型从未见过长程上下文，BatchNorm 统计量基于 800ms 片段，长段推理时累积残差可能导致音质退化。高显存用户（24GB+）可尝试 `segment_ms=1600` 或 `2400`，让模型学习更长上下文，配合推理端的 overlap-add（分短段 vocoder + crossfade 拼接）可进一步消除帧边界伪影。

**overlap-add 推理**：HiFi-GAN 训练时只见过 800ms 短片段，单次推理长音频时处于分布外，帧边界相位跳变是电音的主要来源。overlap-add 将长音频分割为短段（如 3 秒），逐段经 vocoder 重建后再用 crossfade 拼接——让每段落在训练分布内，crossfade 消除边界硬断。开销约 1.5× 推理时间，副作用极低（纯卷积架构无长程状态依赖）。Hanning 窗比线性窗在相位抵消方面好一个数量级。
...
### 如果 RVC 路线不通：DDSP 替代 HiFi-GAN

当前架构是 RVC 变体（HuBERT 内容编码 + HiFi-GAN vocoder）。HiFi-GAN 用 CNN 从特征空间"想象"波形，谐波是 CNN 近似的——音高改动越大，生成的谐波越容易失真（电音）。

DDSP（Differentiable Digital Signal Processing）走了一条不同的路：**

```
输入音频
  ├── F0 → [加法合成器] → 谐波部分（数学上精确的正弦波叠加）
  ├── 响度 → [噪声合成器] → 非周期性部分（气息/齿音）
  └── 合成为最终波形
```

- **谐波部分**：声码器不"生成"波形，而是输出每个谐波的振幅 → 用真实正弦波加法合成。谐波是数学级精确的，无高频假象。
- **噪声部分**：独立噪声解码器处理气声/齿音，不和声学结构混在一起。
- **音高改动**：只改 F0 输入，合成器自动生成新音高下的正确谐波——比 CNN 预测更干净。

如果 HiFi-GAN 训出来电音偏重、活人感不足，可将 vocoder 从 HiFi-GAN 替换为 DDSP 合成器。前端 HuBERT 不变，训练流程不变，改动范围限制在解码器模块。社区已有 DDSP-SVC 等成熟方案可参考。

### DSP/AI 混合策略

回森 APK 分析结果显示，其技术栈为纯 DSP（pyrubberband 同级）+ AI 降噪（1MB TFLite）+ 混响遮瑕。电音和气息不稳问题源于 DSP 大偏移量共振峰失真，而非 vocoder 生成的帧边界伪影。

**两个引擎各有最佳区间**：

| 偏移量 | 引擎 | 理由 |
|--------|------|------|
| < 2 个半音 | DSP | 频域拉伸近乎透明，气息原封不动，零推理开销 |
| > 2 个半音 | AI vocoder | 共振峰不被打扰，不花栗鼠/不怪兽，音色自然 |

混合模式按帧粒度判断：每帧偏移量 < 2 半音走 DSP 管线，≥ 2 半音走 AI vocoder 管线，最终 crossfade 拼接。这样 DSP 管大多数轻修场景（快、透、保气息），AI 管少数重修场景（不拖共振峰），两类 artifact 都不暴露。
